...
(defun solve (board)
  "Solve Sudoku BOARD."
  (foldl
    (lambda (current-board pair)
      (let ((row (car pair))
            (col (cdr pair)))
        (foldl
          (lambda (inner-board n)
            (when (and (= 0 (list-ref inner-board row col))
                       (possible? inner-board row col n))
              ; set value to n and solve if solve returns, continue to next n
              (solve (set-ref inner-board row col n))))
          current-board
          (range 1 (inc 9)))))
    board ; initial condition
    (for-comp ((row (range 9))
               (col (range 9)))
      (cons row col))))
#+end_src
