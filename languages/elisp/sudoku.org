...
(defun solve (board)
  ...
  (catch 'found-solution
    (let ((current-board (copy-sequence board)))
      (unless (some 'identity (mapcar (lambda (row) (member 0 row)) current-board)) ; Check if there are no zeros left
        ...
        (when (= 0 (list-ref current-board row col))
          ...
          (for ((n (range 1 (inc 9))))
            (when (possible? current-board row col n)
              (let ((orig-board (copy-sequence current-board)))
                (setq current-board (set-ref current-board row col n))
                (let ((solution (solve current-board)))
                  (if solution
                      (throw 'found-solution solution)
                    (setq current-board orig-board))))))))
      nil)))
#+end_src
