...
IGNORE```{r, eval=FALSE, tidy = TRUE}
IGNORElibrary(stevemisc)
IGNOREdata(uniondensity)
IGNORE
IGNOREM1 <- lm(union ~ left + size + concen, data=uniondensity)
IGNORE
IGNORElibrary(arm)
IGNOREcoefplot(M1)
IGNORE
...
IGNORE```{r, eval=TRUE, tidy = TRUE, cache=FALSE, fig.cap="A Coefficient Plot", message=F, warning=F}
IGNORElibrary(stevemisc)
IGNOREdata(uniondensity)
IGNORE
IGNOREM1 <- lm(union ~ left + size + concen, data=uniondensity)
...
IGNORE```{r, eval=TRUE, tidy = TRUE, size="small", cache=FALSE, results="asis", message=F, warning=F}
IGNORElibrary(stevemisc)
IGNOREdata(uniondensity)
IGNORElibrary(stargazer)
IGNORE
IGNOREM1 <- lm(union ~ left + size + concen, data=uniondensity)
IGNORE
IGNOREstargazer(M1, title="A Handsome Table", header=FALSE)
IGNORE```
