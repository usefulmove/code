...
state_populations_2018 <- dbGetQuery( # submit SQL query to SQLite library database for state population and region data
               library_db,
               "
               SELECT
                 p.state,
                 ...
                 s.Region AS region
               FROM
                 state_population p LEFT JOIN states s
...
state_deaths_df %>% 
  ...
  ggplot(
    ...
    labs(
      subtitle = "COVID-19 State Deaths",
...
state_deaths_df %>% 
  ...
  ggplot(
    ...
    labs(
      subtitle = "COVID-19 Death Rates by State",
      x = "population (log – in millions)",
...
```{r region_death_rates, out.width = "80%"}
...
region_deaths_df <- state_df %>% 
  ...
  summarise(
    ...
    population = sum(as.integer(population)),
    .groups = "drop"
  ) %>% 
  mutate(
    death_rate = deaths / population
...
region_deaths_df %>% 
  ...
  ggplot(
    ...
    labs(
      subtitle = "COVID-19 Deaths by Region",
      x = "",
      y = "deaths"
    ) +
    scale_fill_nejm()
