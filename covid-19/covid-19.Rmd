...
p9 <- covid("Virginia")
p10 <- covid("Tennessee")
p11 <- covid("Connecticut")
p12 <- covid("Louisiana")
p13 <- covid("Colorado")
p14 <- covid("District of Columbia")
p15 <- covid("Pennsylvania")
p16 <- covid("Massachusetts")

```

<br><br>

```{r disp_state_charts, fig.asp=3.0}
...
us_mobility_pivot %>%
  ...
  ggplot(aes(date, change_from_baseline, group = activity_category)) +
    ...
    annotate(
      geom = "text",
      label = "U.S. declares national emergency on March 13, 2020",
      angle = 90,
