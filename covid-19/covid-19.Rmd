...
world_regions <- decode_read("library", "world_regions")
eu_countries <- decode_read("library", "eu_countries")
gdp <- decode_read("library", "world_gdp") # per capita GDP (PPP) in international dollars
...
state_pop_query <- (
  "
    ...
    ;
  " )
state_populations_2018 <- decode_readSQL("library", state_pop_query)
...
covid <- function(state_name = "United States", time_frame = "all") {
  ...
  print(
    str_glue(
      "{state_name} has {format(s$cases[nrow(s)], big.mark = ',')} cases ",
...
p <- community_mobility |>
  left_join(world_regions, by = c("country_region" = "country")) |>
  filter(
    is.na(sub_region_1),
    is.na(sub_region_2),
