#+title: Chess Org Scratch


#+begin_src emacs-lisp
(load-file "~/repos/cora/src/cora.el") ; load Cora language


(setq fen "5B2/2p5/1rpk4/2NpR1p1/3P1nP1/1P3P2/P1b4P/R5K1 b - - 4 34")


(defun read-fen (fen)
  "Read FEN (Forsyth-Edwards Notation) string and convert it to a board
(list of 8 8-element strings)."
  (thread fen
    'split-string ; (5B2/2p5/1rpk4/2NpR1p1/3P1nP1/1P3P2/P1b4P/R5K1 b - - 4 34)
    (lambda (lst)
      (car (take 1 lst))) ; 5B2/2p5/1rpk4/2NpR1p1/3P1nP1/1P3P2/P1b4P/R5K1
    (lambda (s)
      (string-split s "/")) ; (5B2 2p5 1rpk4 2NpR1p1 3P1nP1 1P3P2 P1b4P R5K1)
    (lambda (lst)
      (map
        (lambda (s) ; "5B2" becomes "_____B__"
          (fold
            (lambda (acc c)
              (cond ((ascii-numeric? c) (let ((count (- c ?0)))
                                          (concat acc
                                                  (join-chars (map
                                                                (lambda (a) ?_)
                                                                (range count))))))
                    (t (concat acc
                               (join-chars (list c))))))
            ""
            (string-to-list s))) ; replace numbers with corresponding number of underscores
        lst)))) ; (_____B__ __p_____ _rpk____ __NpR_p_ ___P_nP_ _P___P__ P_b____P R_____K_)


(defun add-spaces-between (s)
  (fold
    (lambda (acc c)
      (concat acc (join-chars (cons 32 (list c)))))
    ""
    (string-to-list s)))


(defun format-board (board)
  "Format board for print output.
    8 _ _ _ _ _ B _ _
    7 _ _ p _ _ _ _ _
    6 _ r p k _ _ _ _
    5 _ _ N p R _ p _
    4 _ _ _ P _ n P _
    3 _ P _ _ _ P _ P
    2 P _ b _ _ _ _ P
    1 R _ _ _ _ _ K _
    @ a b c d e f g h"
  (cond ((not (and (= 8 (length board))
                   (all? (lambda (row)
                           (= 8 (length row)))
                         board))) (error "board size is incorrect"))
        (t (thread board
             (lambda (rows)
               (map
                 (lambda (row)
                      (concat row "\n"))
                 rows))
             (lambda (rows)
               (map
                 'add-spaces-between
                 rows))
             (lambda (rows)
               (apply 'concat rows))))))


(format-board (read-fen fen))
#+end_src

#+RESULTS:
:  _ _ _ _ _ B _ _
:  _ _ p _ _ _ _ _
:  _ r p k _ _ _ _
:  _ _ N p R _ p _
:  _ _ _ P _ n P _
:  _ P _ _ _ P _ _
:  P _ b _ _ _ _ P
:  R _ _ _ _ _ K _


#+begin_src emacs-lisp
(defun flip-board (board)
  (reverse (thread board
             (lambda (rows)
               (map
                 'reverse
                 rows)))))

(format-board (flip-board board))
#+end_src

#+RESULTS:
:  _ K _ _ _ _ _ R
:  P _ _ _ _ b _ P
:  _ _ P _ _ _ P _
:  _ P n _ P _ _ _
:  _ p _ R p N _ _
:  _ _ _ _ k p r _
:  _ _ _ _ _ p _ _
:  _ _ B _ _ _ _ _
